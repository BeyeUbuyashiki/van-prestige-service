...
router.patch('/users/:id/toggle', authenticate, requireSuperAdmin, (req, res) => {
  const user = db.prepare('SELECT id, is_active FROM admins WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ error: 'Introuvable.' });
  if (user.id === req.admin.id) return res.status(400).json({ error: 'Vous ne pouvez pas désactiver votre propre compte.' });
  const newStatus = user.is_active ? 0 : 1;
  db.prepare('UPDATE admins SET is_active = ? WHERE id = ?').run(newStatus, user.id);
  res.json({ success: true, is_active: newStatus });
});

router.patch('/users/:id/password', authenticate, (req, res) => {
  const { current_password, new_password } = req.body;
  if (req.admin.id !== parseInt(req.params.id) && req.admin.role !== 'superadmin')
    return res.status(403).json({ error: 'Accès refusé.' });
  if (new_password.length < 8) return res.status(400).json({ error: 'Nouveau mot de passe trop court.' });
  ...
  db.prepare('UPDATE admins SET password = ? WHERE id = ?').run(hash, req.params.id);
  res.json({ success: true });
});
